...
import { buildConfusedModePrompt, buildCrisisResponse, detectImminentRisk } from "@/lib/ai/prompts";
...
export async function POST(request: Request) {
  ...
  if (detectImminentRisk(combinedText)) {
    return NextResponse.json({ synthesis: buildCrisisResponse(profile.country), isCrisis: true });
  }
  ...
  const { systemPrompt } = await buildTalkContext({
    ...
    extraInstructions: buildConfusedModePrompt(profile.communication_style, profile.country),
  });
